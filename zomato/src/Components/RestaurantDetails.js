...
const addItem=(item)=>{
 ...
 useEffect(() => {
    fetch(`https://zomato-heroku-app-1.herokuapp.com/zomato/restaurantDetails/${rName}`,{method:'GET'})
    .then(response=>response.json())
    .then(data=>setRestaurant(data.data))
    

    fetch(`https://zomato-heroku-app-1.herokuapp.com/zomato/menu/${rName}`,{method:'GET'})
    .then(response=>response.json())
    .then(data=>{setMenu(data.data);console.log("menu:",menu)})
    }
, [])
...
const loadScript = (src) => {
  ...
  const openRazorPay=async ()=>{
    try{
        let data ;
       data = await fetch("https://zomato-heroku-app-1.herokuapp.com/pay/razorpay", {
            method: "POST",
      ...
      const options = {
        ...
        handler: function (response) {
            var values ={
              ...
              fetch("https://zomato-heroku-app-1.herokuapp.com/pay/transaction", {
                method: "POST",
                headers: { 'Content-Type': 'application/json' },
    ...
    //     <button className="btn btn-danger" style={{ float: 'right', margin: '15px', backgroundColor: '#ce0505' }}onClick={()=>setIsMenuModalOpen(true)}>Place Online Order</button>
    //     <div class="heading">
    //         {name}
    ...
    //                 <div class="value">+91-9448387216</div>
...
<Modal isOpen={isMenuModalOpen} style={modalStyle}>
            ...
            <hr/>
            <h3>Total Price:{totalPrice}</h3><button onClick={()=>{setIsMenuModalOpen(false);loadScript("https://checkout.razorpay.com/v1/checkout.js");openRazorPay(); setIsUserDModalOpen(true)}}>Pay Now</button>
            </Modal>
            </div>
